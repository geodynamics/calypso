...
dnl used as the "Fortran" compiler).  This is not yet solved.
dnl
dnl D*/
dnl
...
[
   ...
   # systems use f2c to implement Fortran, and the version of f2c that they 
...
EOF
   ...
   if test "X$ac_fcompile" = "X" ; then
       ac_fcompile='${FC-fc} -c $FFLAGS conftest.f 1>&AC_FD_CC'
   fi
   ...
   AC_LANG_C   
   save_LIBS="$LIBS"
   dnl FLIBS comes from AC_FC_LIBRARY_LDFLAGS
   LIBS="fconftestf.o $FLIBS $LIBS"
   AC_TRY_LINK([extern void my_name(int);],my_name(0);,pac_cv_prog_fc_name_mangle="lower")
   if test  "X$pac_cv_prog_fc_name_mangle" = "X" ; then
     AC_TRY_LINK([extern void my_name_(int);],my_name_(0);,pac_cv_prog_fc_name_mangle="lower underscore")
   fi
   if test  "X$pac_cv_prog_fc_name_mangle" = "X" ; then
     AC_TRY_LINK([void __stdcall MY_NAME(int);],MY_NAME(0);,pac_cv_prog_fc_name_mangle="upper stdcall")
   fi
   if test  "X$pac_cv_prog_fc_name_mangle" = "X" ; then
     AC_TRY_LINK([extern void MY_NAME(int);],MY_NAME(0);,pac_cv_prog_fc_name_mangle="upper")
   fi
   if test  "X$pac_cv_prog_fc_name_mangle" = "X" ; then
     AC_TRY_LINK([extern void my_name__(int);],my_name__(0);,
       pac_cv_prog_fc_name_mangle="lower doubleunderscore")
   fi
   if test  "X$pac_cv_prog_fc_name_mangle" = "X" ; then
     AC_TRY_LINK([extern void MY_name(int);],MY_name(0);,pac_cv_prog_fc_name_mangle="mixed")
   fi
   if test  "X$pac_cv_prog_fc_name_mangle" = "X" ; then
     AC_TRY_LINK([extern void MY_name_(int);],MY_name_(0);,pac_cv_prog_fc_name_mangle="mixed underscore")
   fi
   LIBS="$save_LIBS"
   AC_LANG_RESTORE
...
EOF
           ...
           AC_LINK_IFELSE([AC_LANG_PROGRAM(,[[        call my_name(0)]])],
	         pac_cv_prog_fc_name_mangle="lower underscore")
       fi
       if test  "X$pac_cv_prog_fc_name_mangle" = "X" ; then
	  # This is needed for Mac OSX 10.5
	  rm -rf conftest.dSYM
...
EOF
           ...
           AC_LINK_IFELSE([AC_LANG_PROGRAM(,[[        call my_name(0)]])],
	         pac_cv_prog_fc_name_mangle="upper stdcall")
       fi
       if test  "X$pac_cv_prog_fc_name_mangle" = "X" ; then
	  # This is needed for Mac OSX 10.5
	  rm -rf conftest.dSYM
...
EOF
           ...
           AC_LINK_IFELSE([AC_LANG_PROGRAM(,[[        call MY_name(0)]])],
	           pac_cv_prog_fc_name_mangle="mixed underscore")
       fi
       LIBS="$save_LIBS"
       AC_LANG_RESTORE
   fi
...
pac_namecheck=`echo X$pac_cv_prog_fc_name_mangle | sed 's/ /-/g'`
ifelse([$1],,[
pac_cv_test_stdcall=""
case $pac_namecheck in
    X) AC_MSG_WARN([Cannot determine Fortran naming scheme]) ;;
    Xlower) AC_DEFINE(FC_NAME_LOWER,1,[Define if Fortran names are lowercase]) 
	FC_NAME_MANGLE="FC_NAME_LOWER"
	;;
    Xlower-underscore) AC_DEFINE(FC_NAME_LOWER_USCORE,1,[Define if Fortran names are lowercase with a trailing underscore])
	FC_NAME_MANGLE="FC_NAME_LOWER_USCORE"
	 ;;
    Xlower-doubleunderscore) AC_DEFINE(FC_NAME_LOWER_2USCORE,1,[Define if Fortran names containing an underscore have two trailing underscores])
	FC_NAME_MANGLE="FC_NAME_LOWER_2USCORE"
	 ;;
    Xupper) AC_DEFINE(FC_NAME_UPPER,1,[Define if Fortran names are uppercase]) 
	FC_NAME_MANGLE="FC_NAME_UPPER"
	;;
    Xmixed) AC_DEFINE(FC_NAME_MIXED,1,[Define if Fortran names preserve the original case]) 
	FC_NAME_MANGLE="FC_NAME_MIXED"
	;;
    Xmixed-underscore) AC_DEFINE(FC_NAME_MIXED_USCORE,1,[Define if Fortran names preserve the original case and add a trailing underscore]) 
	FC_NAME_MANGLE="FC_NAME_MIXED_USCORE"
	;;
    Xupper-stdcall) AC_DEFINE(FC_NAME_UPPER,1,[Define if Fortran names are uppercase])
        FC_NAME_MANGLE="FC_NAME_UPPER_STDCALL"
...
AC_REQUIRE([PAC_PROG_FC_NAME_MANGLE])
# This is needed for Mac OSX 10.5
rm -rf conftest.dSYM
rm -f conftest*
cat <<EOF > conftest.f
...
#define isize_ ISIZE
#elif defined(FC_NAME_LOWER) || defined(FC_NAME_MIXED)
#define cisize_ cisize
#define isize_ isize
#endif
...
EOF
dnl It is important to use the AC_TRY_EVAL in case FC is not a single word
dnl but is something like "FC -64" (where the switch has changed the
dnl compiler)
ac_fscompilelink='${FC-fc} $save_FFLAGS -o conftest conftest.f $LDFLAGS >conftest.bas 2>&1'
ac_fscompilelink2='${FC-fc} $FFLAGS -o conftest conftest.f $LDFLAGS >conftest.out 2>&1'
if AC_TRY_EVAL(ac_fscompilelink) && test -x conftest ; then
   if AC_TRY_EVAL(ac_fscompilelink2) && test -x conftest ; then
      if diff -b conftest.out conftest.bas >/dev/null 2>&1 ; then
         ...
         rm -f conftest2.out
         rm -f conftest.bas
	 ac_fscompile3='${FC-fc} -c $save_FFLAGS conftest2.f >conftest2.out 2>&1'
	 ac_fscompilelink4='${FC-fc} $FFLAGS -o conftest conftest2.o conftest.f $LDFLAGS >conftest.bas 2>&1'
...
EOF
    ...
    if test -z "$ac_fcompilelink" ; then
        ac_fcompilelink="${FC-fc} -o conftest $FFLAGS $flags conftest.f $LDFLAGS $LIBS 1>&AC_FD_CC"
    fi
    AC_MSG_CHECKING([whether ${FC-fc} $flags $libs works with GETARG and IARGC])
    ...
    # Library to link with (none, -lU77 (HPUX), -lg2c (LINUX Fortran))
    # PEPCF90 (Intel ifc)
...
EOF
    ac_fcompilelink_test='${FC-fc} -o conftest $FFLAGS conftest.f $LDFLAGS $libs $LIBS 1>&AC_FD_CC'
    for libs in $save_trial_LIBS ; do
	if test "$libs" = "0" ; then
	    lib_ok="yes"
        else
	    AC_MSG_CHECKING([whether Fortran links with $libs])
...
$libs"
    ...
    # To make the Absoft fortran and f90 work together, we need to prefer the
    ...
    # 
    using_intel_efc="no"
    pac_test_msg=`$FC -V 2>&1 | grep 'Intel(R) Fortran Itanium'`
...
EOF
	    ...
	    AC_MSG_CHECKING([whether Fortran routine names are case-insensitive $flagval])
	    dnl we can use double quotes here because all is already
            dnl evaluated
            ac_fcompilelink_test="${FC-fc} -o conftest $fflag $FFLAGS conftest.f $LDFLAGS $LIBS 1>&AC_FD_CC"
	    if AC_TRY_EVAL(ac_fcompilelink_test) && test -x conftest ; then
	        AC_MSG_RESULT(yes)
	    else
	        AC_MSG_RESULT(no)
...
"
	    ...
	    if test "$libs" = " " -o "$libs" = "0" ; then libs="" ; fi
	        ...
	        if test "$flags" = " " -o "$flags" = "000"; then flags="" ; fi
                AC_MSG_CHECKING([whether ${FC-fc} $flags $libs works with $MSG])
		IFS="$save_IFS"
		dnl We need this here because we've fiddled with IFS
	        ac_fcompilelink_test="${FC-fc} -o conftest $FFLAGS $flags conftest.f $LDFLAGS $libs $LIBS 1>&AC_FD_CC"
		found_answer="no"
                if AC_TRY_EVAL(ac_fcompilelink_test) && test -x conftest ; then
		    if test "$ac_cv_prog_f77_cross" != "yes" ; then
			if ./conftest >/dev/null 2>&1 ; then
...
if test "$found_cached" = "yes" ; then 
    ...
    AC_MSG_WARN([Could not find a way to access the command line from Fortran])
fi
...
AC_CACHE_CHECK([for Fortran flag for library directories],
...
EOF
dnl Build library
    ac_fcompileforlib='${FC-fc} -c $FFLAGS conftest1.f 1>&AC_FD_CC'
	...
	# required for 64-bit objects in some versions of AIX).
        ...
        ac_fcompileldtest='${FC-fc} -o conftest $FFLAGS ${ldir}conftest2 conftest.f -lconftest $LDFLAGS 1>&AC_FD_CC'
...
ac_fcompiletest='${FC-fc} -c $FFLAGS ${idir}$checkdir conftest.f 1>&AC_FD_CC'
...
if AC_TRY_EVAL(ac_compile); then
    ...
    # Solaris.
    if ${FC} ${FFLAGS} -o conftest conftest.o conftest1.o $LDFLAGS 2>&AC_FD_CC ; then
...
EOF
    ...
    tmpcmd='${FC-fc} $FFLAGS -o conftest conftest.f conftestc.o 1>&AC_FD_CC'
    if AC_TRY_EVAL(tmpcmd) && test -x conftest ; then
         pac_cv_prog_f77_and_c_stdio_libs=none
    else
         # Try again with -lSystemStubs
         tmpcmd='${FC-fc} $FFLAGS -o conftest conftest.f conftestc.o -lSystemStubs 1>&AC_FD_CC'
         if AC_TRY_EVAL(tmpcmd) && test -x conftest ; then
             pac_cv_prog_f77_and_c_stdio_libs="-lSystemStubs"
	 else 
	     echo "configure: failed program was:" >&AC_FD_CC
	     cat conftestc.c >&AC_FD_CC 
	     echo "configure: with Fortran program:" >&AC_FD_CC
	     cat conftest.f >&AC_FD_CC
         fi
    fi
...
dnl
dnl Check that the FLIBS determined by AC_FC_LIBRARY_LDFLAGS is valid.
...
AC_DEFUN([PAC_PROG_F77_FLIBS_VALID],[
    pac_cv_fc_flibs_valid=unknown
    AC_MSG_CHECKING([whether $FC accepts the FLIBS found by autoconf])
    AC_LANG_SAVE
    AC_LANG(Fortran)
dnl We can't use TRY_LINK, because it wants a routine name, not a 
dnl declaration.  The following is the body of TRY_LINK, slightly modified.
...
EOF
    if AC_TRY_EVAL(ac_link) && test -s conftest${ac_exeext}; then
      pac_cv_fc_flibs_valid=yes
    else
      echo "configure: failed program was:" >&AC_FD_CC
      cat conftest.$ac_ext >&AC_FD_CC
      pac_cv_fc_flibs_valid=no
    fi
AC_MSG_RESULT($pac_cv_fc_flibs_valid)
if test $pac_cv_fc_flibs_valid = no ; then
...
AC_MSG_CHECKING([whether Fortran and C objects are compatible])
dnl
rm -rf conftestc.dSYM
rm -f conftestc*
dnl construct with a C function with all possible FC name mangling schemes.
...
pac_Fcompile='${FC-fc} -c $FFLAGS conftestf.f 1>&AC_FD_CC'
if AC_TRY_EVAL(pac_Fcompile) && test -s conftestf.${ac_objext} ; then
    pac_f77_working=yes
else
    pac_f77_working=no
    echo "configure: failed FC program was:" >&AC_FD_CC
    cat conftestf.f >&AC_FD_CC
fi
dnl
if test "$pac_c_working" = "yes" -a "$pac_f77_working" = "yes" ; then
dnl Try linking with Fortran compiler
    rm -f conftest${ac_exeext}
    pac_link='$FC $FFLAGS -o conftest${ac_exeext} conftestf.${ac_objext} conftestc.${ac_objext} $LDFLAGS >&AC_FD_CC'
...
Fortran Object File Type produced by \"${FC} ${FFLAGS}\" is : ${fobjtype}.
