...
!!
!!@verbatim
!!      subroutine gz_read_domain_info(FPz_f, id_rank,                  &
!!     &                               comm_IO, zbuf, ierr)
!!      subroutine gz_read_import_data(FPz_f, comm_IO, zbuf)
!!      subroutine gz_read_export_data(FPz_f, comm_IO, zbuf)
!!        character, pointer, intent(in) :: FPz_f
!!        type(communication_table), intent(inout) :: comm_IO
!!        type(buffer_4_gzip) , intent(inout):: zbuf
!!
!!      subroutine gz_write_domain_info(FPz_f, id_rank, comm_IO, zbuf)
!!      subroutine gz_write_import_data(FPz_f, comm_IO, zbuf)
!!      subroutine gz_write_export_data(FPz_f, comm_IO, zbuf)
!!        character, pointer, intent(in) :: FPz_f
...
!
     ...
     &     (FPz_f, comm_IO%num_neib, comm_IO%id_neib, zbuf)
      end if
!
...
!
        ...
        call read_send_recv_item_gz                                     &
     &     (FPz_f, comm_IO%ntot_export, comm_IO%item_export, zbuf)
      else
...
!
     ...
     &   (FPz_f, comm_IO%num_neib, comm_IO%ntot_import,                 &
...
!
      call write_send_recv_data_gz                                      &
     &   (FPz_f, comm_IO%num_neib, comm_IO%ntot_export,                 &
...
! -----------------------------------------------------------------------
!
      subroutine read_send_recv_item_gz(FPz_f, ntot_sr, inod_sr, zbuf)
!
      use gzip_file_access
      use skip_gz_comment
!
...
!
      integer(kind = kint) :: i
!
      call skip_gz_comment_int(FPz_f, inod_sr(1), zbuf)
      do i = 2, ntot_sr
        call get_one_line_text_from_gz(FPz_f, zbuf)
...
!
     ...
     &         (FPz_f, num_sr, ntot_sr, istack_sr, inod_sr, zbuf)
!
      use gzip_file_access
      use gz_data_IO
!
...
!
      if (num_sr .gt. 0) then
        call write_gz_multi_int_8i16(FPz_f, num_sr, istack_sr(1), zbuf)
        do i = 1, ntot_sr
          write(zbuf%fixbuf(1),'(i16,2a1)')                             &
