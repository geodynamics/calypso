...
\item{\tt  control\_MHD}		Control file for simulation
\item{\tt control\_snapshot}	Control file for postprocessing
\item{\tt control\_psf\_CMB}	Control file for section at CMB
\item{\tt control\_psf\_eq}		Control file for section at equatorial plane
\item{\tt control\_psf\_z0.3}	Control file for section at z = 0.3 
\item{\tt control\_psf\_s0.55}	Control file for cylindrical surface at s = 0.55
\item{\tt control\_iso\_temp}	Control file for isosurface of temperature
\item{\tt sph\_lm31r48c\_4} 	Spherical shell indexing data directory
\item{\tt rst\_4}				Spectr data directory for restarting
\item{\tt field}				Field data directory for for visualization
\item{\tt field}				Field data directory for for visualization
\item{\tt setions}			Cross section data directory for for visualization
\item{\tt isourfaces}			Isosurface data directory for for visualization
\end{description}
%
...
\item{\tt control\_snapshot}	Control file for postprocessing
\item{\tt control\_psf\_CMB}	Control file for section at CMB
\item{\tt control\_psf\_ICB}		Control file for section at ICB
\item{\tt control\_psf\_eq}		Control file for section at equatorial plane
\item{\tt control\_psf\_z0.3}	Control file for section at z = 0.3 
\item{\tt control\_psf\_s0.55}	Control file for cylindrical surface at s = 0.55
\item{\tt sph\_lm31r48c\_4} 	Spherical shell indexing data directory
...
$q_{T} = \frac{2}{r}$ and $q_{C} = 1.0$, respectively. The source terms are given in the initial field data. To generate initial data, 
