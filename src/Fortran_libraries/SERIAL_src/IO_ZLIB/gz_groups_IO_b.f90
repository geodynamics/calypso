...
!!
!!@verbatim
!!      subroutine gz_read_group_data_b(zbuf, group_IO)
!!      subroutine gz_read_surf_grp_data_b(zbuf, surf_grp_IO)
...
!
!
      call gz_read_one_integer_b(zbuf, group_IO%num_grp)
...
!
     ...
     &     (zbuf, cast_long(group_IO%num_grp),                          &
     &      group_IO%istack_grp, group_IO%num_item)
        if(zbuf%ierr_zlib .ne. 0) return
!
        call gz_read_mul_character_b                                    &
     &     (zbuf, cast_long(group_IO%num_grp), group_IO%grp_name)
        if(zbuf%ierr_zlib .ne. 0) return
!
        call alloc_group_item(group_IO)
!
        call gz_read_mul_integer_b                                      &
     &     (zbuf, cast_long(group_IO%num_item), group_IO%item_grp)
        if(zbuf%ierr_zlib .ne. 0) return
      else
        group_IO%num_item = 0
        call alloc_group_item(group_IO)
...
!
!
      call gz_read_one_integer_b(zbuf, surf_grp_IO%num_grp)
      if(zbuf%ierr_zlib .ne. 0) return
!
...
!
      if (surf_grp_IO%num_grp .gt. 0) then
        call gz_read_integer_stack_b                                    &
     &     (zbuf, cast_long(surf_grp_IO%num_grp),                       &
     &      surf_grp_IO%istack_grp, surf_grp_IO%num_item)
        if(zbuf%ierr_zlib .ne. 0) return
!
        call gz_read_mul_character_b                                    &
     &     (zbuf, cast_long(surf_grp_IO%num_grp), surf_grp_IO%grp_name)
        if(zbuf%ierr_zlib .ne. 0) return
!
...
!
        ...
        call gz_read_mul_integer_b                                      &
     &     (zbuf, nitem, surf_grp_IO%item_sf_grp)
        if(zbuf%ierr_zlib .ne. 0) return
      else
        call alloc_sf_group_item(surf_grp_IO)
      end if
