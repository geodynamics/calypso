...
!
!
      if(check_field_list_ctl(temperature_from_CMB, field_ctl)) then
        call add_phys_name_ctl(temperature, field_ctl)
      end if
!
      if(check_field_list_ctl(composition_from_CMB, field_ctl)) then
        call add_phys_name_ctl(composition, field_ctl)
      end if
!
      if(check_field_list_ctl(density_from_CMB, field_ctl)) then
        call add_phys_name_ctl(density, field_ctl)
      end if
!
      if(check_field_list_ctl(entropy_from_CMB, field_ctl)) then
        call add_phys_name_ctl(entropy, field_ctl)
      end if
!
      if(   check_field_list_ctl(pressure_from_CMB, field_ctl)          &
     & .or. check_field_list_ctl(aspherical_pressure, field_ctl)        &
     &   )  then
