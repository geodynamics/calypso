...
!C***DESCRIPTION
!C
!C   ISORT_w_INT sorts array IX and optionally makes the same interchanges in
...
!C***END PROLOGUE  ISORT_w_INT
!
      module isort_with_int
!
      use m_precision
!
      implicit none
...
!-----------------------------------------------------------------------
!
      SUBROUTINE ISORT_w_INT (IX, IY, N, KFLAG)
!C     .. Scalar Arguments ..
      INTEGER(kind = kint) ::  KFLAG, N
!C     .. Array Arguments ..
      INTEGER(kind = kint) ::  IX(N), IY(N)
!C     .. Local Scalars ..
      REAL R
      INTEGER(kind = kint) ::  I, IJ, J, K, KK, L, M, NN, T, TT, TTY, TY
!C     .. Local Arrays ..
...
!C     .. Intrinsic Functions ..
      INTRINSIC ABS, INT
!C***FIRST EXECUTABLE STATEMENT  ISORT_w_INT
      NN = N
      IF (NN .LT. 1) THEN
!         CALL XERMSG ('SLATEC', 'ISORT_w_INT',
...
!C
  ...
  200    CONTINUE
      ENDIF
      RETURN
      END SUBROUTINE ISORT_w_INT
!
!-----------------------------------------------------------------------
!
      end module isort_with_int
