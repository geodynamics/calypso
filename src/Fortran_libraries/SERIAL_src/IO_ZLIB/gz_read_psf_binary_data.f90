...
!!     &         (FPz_f, np_read, nnod, num_field, zbuf)
!!      subroutine read_psf_phys_name_bin_gz(FPz_f, num_field,          &
!!     &          ntot_comp, ncomp_field, field_name, zbuf)
!!      subroutine read_psf_phys_data_bin_gz                            &
!!     &         (FPz_f, np_read, nnod, ntot_comp, d_nod, zbuf)
!!        character, pointer, intent(in) :: FPz_f
!!
!!      subroutine read_psf_node_num_bin_gz(FPz_f, np_read, nnod, zbuf)
!!      subroutine read_psf_node_data_bin_gz                            &
!!     &         (FPz_f, np_read, nnod, inod_global, xx, zbuf)
!!        character, pointer, intent(in) :: FPz_f
!!
!!      subroutine read_psf_ele_num_bin_gz                              &
!!     &         (FPz_f, np_read, nele, nnod_ele, zbuf)
!!      subroutine read_psf_ele_connect_bin_gz(FPz_f, np_read,          &
!!     &          nele, nnod_ele, iele_global, ie_psf, zbuf)
!!        character, pointer, intent(in) :: FPz_f
...
!
!
      call read_psf_node_num_bin_gz(FPz_f, np_read, nnod_gl, zbuf)
...
!
     ...
     &   (FPz_f, zbuf, cast_long(num_field), ncomp_field)
...
!
     ...
     &                          cast_long(np_read), itmp1_mp_gz)
        call gz_read_1d_vector_b(FPz_f, zbuf, nnod, d_nod(1,nd))
      end do
...
!
      ...
      call gz_read_mul_int8_b(FPz_f, zbuf, cast_long(np_read), n_inter)
      call gz_read_mul_int8_b(FPz_f, zbuf, cast_long(np_read), n_inter)
      nnod = sum(n_inter)
!
...
!
     ...
     &   (FPz_f, np_read, nnod, ithree, xx, zbuf)
!
!$omp parallel do
      do i = 1, nnod
        inod_global(i) = i
...
!
!
      call gz_read_mul_int8_b(FPz_f, zbuf, cast_long(ione), nnod_ele_b)
      call gz_read_mul_int8_b(FPz_f, zbuf, cast_long(ione), eletype)
!
      allocate(nele_lc(np_read))
      call gz_read_mul_int8_b(FPz_f, zbuf, cast_long(np_read), nele_lc)
      call gz_read_mul_int8_b(FPz_f, zbuf, cast_long(np_read), nele_lc)
!
...
!
      ...
      do nd = 1, nnod_ele
        call gz_read_mul_int8_b(FPz_f, zbuf,                            &
     &                          cast_long(np_read), itmp1_mp_gz)
        call gz_read_mul_int8_b(FPz_f, zbuf, nele, ie_psf(1,nd))
      end do
      deallocate(itmp1_mp_gz)
!
!$omp parallel do
