!>@file   gz_layer_spectr_monitor_IO.F90
!!
!! @author H. Matsui
!! @date   Programmed in  Nov., 2007
!!         modified in Sep., 2022
!!
!> @brief  Layerd mean square data output
!!
!!@verbatim
!!      subroutine swap_layer_spectr_to_IO(nri_sph, ltr, ntot_comp,     &
!!     &                                   rms_sph, spectr_IO)
!!        integer(kind = kint), intent(in) :: nri_sph, ltr, ntot_comp
!!        real(kind = kreal), intent(in)                                &
!!     &                   :: rms_sph(nri_sph,0:ltr,ntot_comp)
!!        real(kind = kreal), intent(inout)                             &
!!     &                   :: spectr_IO(ntot_comp,0:ltr,nri_sph)
!!      subroutine sel_gz_read_layer_spectr_mtr                         &
!!     &         (FPz_f, id_stream, flag_gzip, nri_sph, ltr, ntot_comp, &
!!     &          i_step, time, kr_sph, r_sph, i_mode,                  &
!!     &          spectr_IO, zbuf, ierr)
!!        character, pointer, intent(in) :: FPz_f
!!        integer(kind = kint), intent(in) :: id_stream
!!        logical, intent(in) :: flag_gzip
!!        integer(kind = kint), intent(in) :: nri_sph, ltr
!!        integer(kind = kint), intent(in) :: ntot_comp
!!        integer(kind = kint), intent(inout) :: i_step
!!        real(kind = kreal), intent(inout) :: time
!!        integer(kind = kint), intent(inout) :: i_mode(0:ltr)
!!        integer(kind = kint), intent(inout) :: kr_sph(nri_sph)
!!        real(kind = kreal), intent(inout) :: r_sph(nri_sph)
!!        real(kind = kreal), intent(inout)                             &
!!     &                   :: spectr_IO(ntot_comp,0:ltr,nri_sph)
!!        integer(kind = kint), intent(inout) :: ierr
!!        type(buffer_4_gzip), intent(inout) :: zbuf
!!      subroutine sel_gz_write_layer_spectr_mtr(flag_gzip, id_file,    &
!!     &          i_step, time, nri_sph, kr_sph, r_sph, ltr, ntot_comp, &
!!     &          spectr_IO, zbuf)
!!        logical, intent(in) :: flag_gzip
!!        integer(kind = kint), intent(in) :: id_file
!!        integer(kind = kint), intent(in) :: i_step
!!        integer(kind = kint), intent(in) :: nri_sph, ltr
!!        integer(kind = kint), intent(in) :: kr_sph(nri_sph)
!!        real(kind = kreal), intent(in) :: time
!!        real(kind = kreal), intent(in) :: r_sph(nri_sph)
!!        integer(kind = kint), intent(in) :: ntot_comp
!!        real(kind = kreal), intent(in)                                &
!!     &                   :: spectr_IO(ntot_comp,0:ltr,nri_sph)
!!        type(buffer_4_gzip), intent(inout) :: zbuf
!!@endverbatim
      module gz_layer_spectr_monitor_IO
!
      use m_precision
      use m_constants
      use t_buffer_4_gzip
!
      implicit none
!
      private :: gz_write_layer_spectr_monitor
!
! -----------------------------------------------------------------------
!
      contains
!
! -----------------------------------------------------------------------
!
      subroutine swap_layer_spectr_to_IO(nri_sph, ltr, ntot_comp,       &
     &                                   rms_sph, spectr_IO)
!
      integer(kind = kint), intent(in) :: nri_sph, ltr, ntot_comp
      real(kind = kreal), intent(in)                                    &
     &                   :: rms_sph(nri_sph,0:ltr,ntot_comp)
      real(kind = kreal), intent(inout)                                 &
     &                   :: spectr_IO(ntot_comp,0:ltr,nri_sph)
!
      integer(kind = kint) :: kr, l
!
!
!$omp parallel do private(kr,l)
      do kr = 1, nri_sph
        do l = 0, ltr
          spectr_IO(1:ntot_comp,l,kr) = rms_sph(kr,l,1:ntot_comp)
        end do
      end do
!$omp end parallel do
!
      end subroutine swap_layer_spectr_to_IO
!
! -----------------------------------------------------------------------
! -----------------------------------------------------------------------
!
      subroutine sel_gz_read_layer_spectr_mtr                           &
     &         (FPz_f, id_stream, flag_gzip, nri_sph, ltr, ntot_comp,   &
     &          i_step, time, kr_sph, r_sph, i_mode,                    &
     &          spectr_IO, zbuf, ierr)
!
      use select_gz_stream_file_IO
!
      character, pointer, intent(in) :: FPz_f
      integer(kind = kint), intent(in) :: id_stream
      logical, intent(in) :: flag_gzip
      integer(kind = kint), intent(in) :: nri_sph, ltr
      integer(kind = kint), intent(in) :: ntot_comp
!
      integer(kind = kint), intent(inout) :: i_step
      real(kind = kreal), intent(inout) :: time
      integer(kind = kint), intent(inout) :: i_mode(0:ltr)
      integer(kind = kint), intent(inout) :: kr_sph(nri_sph)
      real(kind = kreal), intent(inout) :: r_sph(nri_sph)
      real(kind = kreal), intent(inout)                                 &
     &                   :: spectr_IO(ntot_comp,0:ltr,nri_sph)
      integer(kind = kint), intent(inout) :: ierr
      type(buffer_4_gzip), intent(inout) :: zbuf
!
      integer(kind = kint) :: l, kr
!
!
      ierr = 1
      do kr = 1, nri_sph
        do l = 0, ltr
          call sel_read_line_gz_stream(FPz_f, id_stream,                &
     &                                 flag_gzip, zbuf)
          if(zbuf%len_used .lt. 0) return
!
          read(zbuf%fixbuf(1),*,err=99) i_step, time,                   &
     &        kr_sph(kr), r_sph(kr), i_mode(l),                         &
     &        spectr_IO(1:ntot_comp,l,kr)
        end do
      end do
      ierr = 0
      return
!
   99 continue
      return
!
      end subroutine sel_gz_read_layer_spectr_mtr
!
!   --------------------------------------------------------------------
!  --------------------------------------------------------------------
!
      subroutine sel_gz_write_layer_spectr_mtr(flag_gzip, id_file,      &
     &          i_step, time, nri_sph, kr_sph, r_sph, ltr, ntot_comp,   &
     &          spectr_IO, zbuf)
!
      use sph_monitor_data_text
!
      logical, intent(in) :: flag_gzip
      integer(kind = kint), intent(in) :: id_file
      integer(kind = kint), intent(in) :: i_step
      integer(kind = kint), intent(in) :: nri_sph, ltr
      integer(kind = kint), intent(in) :: kr_sph(nri_sph)
      real(kind = kreal), intent(in) :: time
      real(kind = kreal), intent(in) :: r_sph(nri_sph)
      integer(kind = kint), intent(in) :: ntot_comp
      real(kind = kreal), intent(in)                                    &
     &                   :: spectr_IO(ntot_comp,0:ltr,nri_sph)
!
      type(buffer_4_gzip), intent(inout) :: zbuf
!
      integer(kind = kint) :: l, k
!
!
#ifdef ZLIB_IO
      if(flag_gzip) then
        call gz_write_layer_spectr_monitor                              &
     &     (id_file, i_step, time, nri_sph, kr_sph, r_sph, ltr,         &
     &      ntot_comp, spectr_IO, zbuf)
        return
      end if
#endif
!
      do k = 1, nri_sph
        do l = 0, ltr
          write(id_file) layer_spectr_data_text(i_step, time,           &
     &                                          kr_sph(k), r_sph(k),    &
     &                                          l, ntot_comp,           &
     &                                          spectr_IO(1,l,k))
        end do
      end do
!
      end subroutine sel_gz_write_layer_spectr_mtr
!
! -----------------------------------------------------------------------
! -----------------------------------------------------------------------
!
#ifdef ZLIB_IO
      subroutine gz_write_layer_spectr_monitor                          &
     &         (id_file, i_step, time, nri_sph, kr_sph, r_sph, ltr,     &
     &          ntot_comp, spectr_IO, zbuf)
!
      use sph_monitor_data_text
      use gzip_defleate
!
      integer(kind = kint), intent(in) :: id_file
      integer(kind = kint), intent(in) :: i_step
      integer(kind = kint), intent(in) :: nri_sph, ltr
      integer(kind = kint), intent(in) :: kr_sph(nri_sph)
      real(kind = kreal), intent(in) :: time
      real(kind = kreal), intent(in) :: r_sph(nri_sph)
      integer(kind = kint), intent(in) :: ntot_comp
      real(kind = kreal), intent(in)                                    &
     &                   :: spectr_IO(ntot_comp,0:ltr,nri_sph)
!
      type(buffer_4_gzip), intent(inout) :: zbuf
!
      integer(kind = kint) :: l, k, line_len
!
!
      line_len = len(layer_spectr_data_text(i_step, time, kr_sph(1),    &
     &                                    r_sph(1), izero,              &
     &                                    ntot_comp, spectr_IO(1,0,1)))
      zbuf%ilen_gz = int(dble((ltr+1)*line_len)*1.01 + 24,              &
     &                   KIND(zbuf%ilen_gz))
      call alloc_zip_buffer(zbuf)
!
      do k = 1, nri_sph
        zbuf%ilen_gzipped = 0
        call gzip_defleat_char_begin(line_len,                          &
     &      layer_spectr_data_text(i_step, time, kr_sph(k), r_sph(k),   &
     &                             izero, ntot_comp, spectr_IO(1,0,k)), &
     &      int(zbuf%ilen_gz), zbuf, zbuf%gzip_buf(1))
        do l = 1, ltr - 1
          call gzip_defleat_char_cont(line_len,                         &
     &        layer_spectr_data_text(i_step, time, kr_sph(k), r_sph(k), &
     &                               l, ntot_comp, spectr_IO(1,l,k)),   &
     &        zbuf)
        end do
        call gzip_defleat_char_last(line_len,                           &
     &      layer_spectr_data_text(i_step, time, kr_sph(k), r_sph(k),   &
     &                             ltr, ntot_comp, spectr_IO(1,ltr,k)), &
     &      zbuf)
!
        write(id_file) zbuf%gzip_buf(1:zbuf%ilen_gzipped)
      end do
      call dealloc_zip_buffer(zbuf)
!
      end subroutine gz_write_layer_spectr_monitor
#endif
!
! -----------------------------------------------------------------------
!
      end module gz_layer_spectr_monitor_IO
