...
!
     ...
     &         = sp_rlm_spin(j_rlm,kr_nd           )                    &
     &           + vr_rtm_spin(l_rtm,mp_rlm,kr_nd         ) * pwt_tmp
!
              sp_rlm_spin(j_rlm,kr_nd+nb_nri  )                         &
     &         = sp_rlm_spin(j_rlm,kr_nd+nb_nri  )                      &
     &         + ( vr_rtm_spin(l_rtm,mp_rlm,kr_nd+nb_nri  ) * dpwt_tmp  &
     &           - vr_rtm_spin(l_rtm,mn_rlm,kr_nd+2*nb_nri) * pgwt_tmp)
!
              sp_rlm_spin(j_rlm,kr_nd+2*nb_nri)                         &
     &         = sp_rlm_spin(j_rlm,kr_nd+2*nb_nri)                      &
     &         - ( vr_rtm_spin(l_rtm,mn_rlm,kr_nd+nb_nri  ) * pgwt_tmp  &
     &           + vr_rtm_spin(l_rtm,mp_rlm,kr_nd+2*nb_nri) * dpwt_tmp)
            end do
          end do
        end do
      end do
