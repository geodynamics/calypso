...
!          st_elapsed = MPI_WTIME()
        call set_vr_rtm_vec_sym_inner_omp                               &
     ...
     &      WK_l_tst%asmp_r, WK_l_tst%symp_p)
        call set_vr_rtm_scl_sym_inner_omp                               &
     &     (sph_rtm%nnod_rtm, sph_rtm%nidx_rtm, sph_rtm%istep_rtm,      &
     &      sph_rlm%nidx_rlm, weight_rtm,                               &
     &      np_smp, WK_l_tst%istack_nlo_rtm, WK_l_tst%narray_nl,        &
     &      WK_l_tst%nl_e_rtm, mp_rlm,                                  &
     &      ncomp, nvector, nscalar, comm_rtm%irev_sr, n_WR, WR,        &
     &      WK_l_tst%symp_r, WK_l_tst%asmp_r)
...
!$omp parallel do
     ...
     &       (nkrs, n_jk_e, WK_l_tst%narray_nl, WK_l_tst%symp_r(1,ip),  &
     &        WK_l_tst%Ps_tj(1,jst+1,ip),    WK_l_tst%pol_e_smp(1,ip))
          call matmul_fwd_leg_trans                                     &
     &       (nkrt, n_jk_e, WK_l_tst%narray_nl, WK_l_tst%asmp_p(1,ip),  &
     &        WK_l_tst%dPsdt_tj(1,jst+1,ip), WK_l_tst%tor_e_smp(1,ip))
!
!  odd l-m
          call matmul_fwd_leg_trans                                     &
     &       (nkrs, n_jk_o, WK_l_tst%narray_nl, WK_l_tst%asmp_r(1,ip),  &
     &        WK_l_tst%Ps_tj(1,jst_h,ip),    WK_l_tst%pol_o_smp(1,ip))
          call matmul_fwd_leg_trans                                     &
     &       (nkrt, n_jk_o, WK_l_tst%narray_nl, WK_l_tst%symp_p(1,ip),  &
     &        WK_l_tst%dPsdt_tj(1,jst_h,ip), WK_l_tst%tor_o_smp(1,ip))
        end do
!$omp end parallel do
!
!$omp parallel
!$omp workshare
        WK_l_tst%pol_e(1:WK_l_tst%n_pol_e) = 0.0d0
        WK_l_tst%tor_e(1:WK_l_tst%n_tor_e) = 0.0d0
        WK_l_tst%pol_o(1:WK_l_tst%n_pol_e) = 0.0d0
        WK_l_tst%tor_o(1:WK_l_tst%n_tor_e) = 0.0d0
!$omp end workshare
        do ip = 1, np_smp
!$omp workshare
          WK_l_tst%pol_e(1:WK_l_tst%n_pol_e)                            &
     &      = WK_l_tst%pol_e(1:WK_l_tst%n_pol_e)                        &
     &       + WK_l_tst%pol_e_smp(1:WK_l_tst%n_pol_e,ip)
          WK_l_tst%tor_e(1:WK_l_tst%n_tor_e)                            &
     &      = WK_l_tst%tor_e(1:WK_l_tst%n_tor_e)                        &
     &       + WK_l_tst%tor_e_smp(1:WK_l_tst%n_tor_e,ip)
          WK_l_tst%pol_o(1:WK_l_tst%n_pol_e)                            &
     &      = WK_l_tst%pol_o(1:WK_l_tst%n_pol_e)                        &
     &       + WK_l_tst%pol_o_smp(1:WK_l_tst%n_pol_e,ip)
          WK_l_tst%tor_o(1:WK_l_tst%n_tor_e)                            &
     &      = WK_l_tst%tor_o(1:WK_l_tst%n_tor_e)                        &
     &       + WK_l_tst%tor_o_smp(1:WK_l_tst%n_tor_e,ip)
!$omp end workshare
        end do
!$omp end parallel
!
...
!
!          st_elapsed = MPI_WTIME()
        call cal_sp_rlm_vec_sym_inner_omp                               &
     &     (sph_rlm%nnod_rlm, sph_rlm%nidx_rlm,                         &
     &      sph_rlm%istep_rlm, sph_rlm%idx_gl_1d_rlm_j,                 &
     &      sph_rlm%radius_1d_rlm_r, g_sph_rlm, jst, n_jk_o, n_jk_e,    &
     &      WK_l_tst%pol_e, WK_l_tst%pol_o,                             &
     &      WK_l_tst%tor_e, WK_l_tst%tor_o,                             &
     &      ncomp, nvector, comm_rlm%irev_sr, n_WS, WS)
        call cal_sp_rlm_scl_sym_inner_omp(sph_rlm%nnod_rlm,             &
     &      sph_rlm%nidx_rlm, sph_rlm%istep_rlm, g_sph_rlm,             &
     &      jst, n_jk_o, n_jk_e, WK_l_tst%pol_e, WK_l_tst%pol_o,        &
...
! -----------------------------------------------------------------------
!
      subroutine set_vr_rtm_vec_sym_inner_omp(nnod_rtm, nidx_rtm,       &
     &          istep_rtm, nidx_rlm, asin_theta_1d_rtm, weight_rtm,     &
     &          np_smp, istack_nlo_rtm, narray_nl, nl_e_rtm,            &
     &          mp_rlm, mn_rlm, ncomp, nvector, irev_sr_rtm,            &
     &          n_WR, WR, symp_r, asmp_p, asmp_r, symp_p)
!
...
!
     ...
     &               :: symp_r(ncomp*nidx_rlm(1),    narray_nl,np_smp)
      real(kind = kreal), intent(inout)                                 &
     &               :: asmp_p(2*nidx_rlm(1)*nvector,narray_nl,np_smp)
      real(kind = kreal), intent(inout)                                 &
     &               :: asmp_r(ncomp*nidx_rlm(1),    narray_nl,np_smp)
      real(kind = kreal), intent(inout)                                 &
     &               :: symp_p(2*nidx_rlm(1)*nvector,narray_nl,np_smp)
!
!
      integer(kind = kint) :: ip, ist, num, lnum
...
!$omp parallel do private(ip,ist,num,lnum,lp_rtm,ln_rtm,wp_rtm,         &
!$omp&        asin_rtm,kk,k_rlm,nd,ip_rtpm,ip_rtnm,in_rtpm,in_rtnm,     &
!$omp&        ipp_recv,ipn_recv,inp_recv,inn_recv)
      do ip = 1, np_smp
        ist = istack_nlo_rtm(ip-1)
...
!
            asmp_r(kk,lnum,ip)                                          &
     &          = (WR(ipp_recv-2) - WR(ipn_recv-2)) * wp_rtm
            asmp_p(kk+nkrv,lnum,ip)                                     &
     &          = (WR(ipp_recv-1) - WR(ipn_recv-1)) * wp_rtm
            asmp_p(kk,lnum,ip)                                          &
     &          = (WR(ipp_recv  ) - WR(ipn_recv  )) * wp_rtm
!
            symp_r(kk+nkrv,lnum,ip)                                     &
     &          = (WR(inp_recv-1) + WR(inn_recv-1)) * wp_rtm * asin_rtm
            symp_r(kk+2*nkrv,lnum,ip)                                   &
     &          = (WR(inp_recv  ) + WR(inn_recv  )) * wp_rtm * asin_rtm
!
            asmp_r(kk+nkrv,lnum,ip)                                     &
     &          = (WR(inp_recv-1) - WR(inn_recv-1)) * wp_rtm * asin_rtm
            asmp_r(kk+2*nkrv,lnum,ip)                                   &
     &          = (WR(inp_recv  ) - WR(inn_recv  )) * wp_rtm * asin_rtm
          end do
        end do
!
!     Unused area
        ist = istack_nlo_rtm(ip)
        num = narray_nl - istack_nlo_rtm(ip)
        do lnum = 1, num
          do kk = 1, nkrv
            symp_r(kk,lnum,ip) =        0.0d0
            symp_p(kk+nkrv,lnum,ip) =   0.0d0
            symp_p(kk,lnum,ip) =        0.0d0
!
            asmp_r(kk,lnum,ip) =        0.0d0
            asmp_p(kk+nkrv,lnum,ip) =   0.0d0
            asmp_p(kk,lnum,ip) =        0.0d0
!
            symp_r(kk+nkrv,lnum,ip) =   0.0d0
            symp_r(kk+2*nkrv,lnum,ip) = 0.0d0
!
            asmp_r(kk+nkrv,lnum,ip) =   0.0d0
            asmp_r(kk+2*nkrv,lnum,ip) = 0.0d0
          end do
        end do
      end do
!$omp end parallel do
...
!   Equator (if necessary)
      if(nl_e_rtm .gt. istack_nlo_rtm(np_smp)) then
        ...
        wp_rtm = weight_rtm(nl_e_rtm)
        asin_rtm = asin_theta_1d_rtm(nl_e_rtm)
!$omp parallel do                                                       &
!$omp& private(kk,k_rlm,nd,ip_rtpm,in_rtpm,ipp_recv,inp_recv)
        do kk = 1, nkrv
          k_rlm = 1 + mod((kk-1),nidx_rlm(1))
          nd = 1 + (kk - k_rlm) / nidx_rlm(1)
!
     ...
     &                + (mn_rlm-1) * istep_rtm(3)
          ...
          inp_recv = 3*nd + (irev_sr_rtm(in_rtpm) - 1) * ncomp
!
          symp_r(kk,lnum,np_smp) =      WR(ipp_recv-2) * wp_rtm
          symp_p(kk+nkrv,lnum,np_smp) = WR(ipp_recv-1) * wp_rtm
          symp_p(kk,lnum,np_smp) =      WR(ipp_recv  ) * wp_rtm
!
          asmp_r(kk,lnum,np_smp) =      0.0d0
          asmp_p(kk+nkrv,lnum,np_smp) = 0.0d0
          asmp_p(kk,lnum,np_smp) =      0.0d0
!
          symp_r(kk+nkrv,lnum,np_smp)                                   &
     &          = WR(inp_recv-1) * wp_rtm * asin_rtm
          symp_r(kk+2*nkrv,lnum,np_smp)                                 &
     &          = WR(inp_recv  ) * wp_rtm * asin_rtm
!
          asmp_r(kk+nkrv,lnum,np_smp) =   0.0d0
          asmp_r(kk+2*nkrv,lnum,np_smp) = 0.0d0
        end do
      end if
!
      end subroutine set_vr_rtm_vec_sym_inner_omp
!
! -----------------------------------------------------------------------
!
      subroutine set_vr_rtm_scl_sym_inner_omp                           &
     &         (nnod_rtm, nidx_rtm, istep_rtm, nidx_rlm, weight_rtm,    &
     &          np_smp, istack_nlo_rtm, narray_nl, nl_e_rtm, mp_rlm,    &
...
!
      real(kind = kreal), intent(inout)                                 &
     &                   :: symp(ncomp*nidx_rlm(1),narray_nl,np_smp)
      real(kind = kreal), intent(inout)                                 &
     &                   :: asmp(ncomp*nidx_rlm(1),narray_nl,np_smp)
!
      integer(kind = kint) :: ip, ist, num, lnum
      integer(kind = kint) :: kk, k_rlm, nd
      integer(kind = kint) :: lp_rtm, ln_rtm, nkrv
...
!
      nkrv = nidx_rlm(1) * nvector
!$omp parallel do private(ip,ist,num,lnum,lp_rtm,ln_rtm,wp_rtm,         &
!$omp&        kk,k_rlm,nd,ip_rtpm,ip_rtnm,ipp_recv,ipn_recv)
      do ip = 1, np_smp
        ist = istack_nlo_rtm(ip-1)
        num = istack_nlo_rtm(ip) - istack_nlo_rtm(ip-1)
        do lnum = 1, num
...
!
            symp(kk+3*nkrv,lnum,ip) = (WR(ipp_recv) + WR(ipn_recv))     &
     &                               * wp_rtm
            asmp(kk+3*nkrv,lnum,ip) = (WR(ipp_recv) - WR(ipn_recv))     &
     &                               * wp_rtm
          end do
        end do
!
!     Unused area
        ...
        do lnum = 1, num
          do kk = 1, nidx_rlm(1)*nscalar
            symp(kk+3*nkrv,lnum,ip) = 0.0d0
            asmp(kk+3*nkrv,lnum,ip) = 0.0d0
          end do
        end do
      end do
!$omp end parallel do
...
!   Equator (if necessary)
      if(nl_e_rtm .gt. istack_nlo_rtm(np_smp)) then
        ...
        lp_rtm = nl_e_rtm
        wp_rtm = weight_rtm(lp_rtm)
!$omp parallel do private(kk,k_rlm,nd,ip_rtpm,ipp_recv)
        do kk = 1, nidx_rlm(1)*nscalar
          k_rlm = 1 + mod((kk-1),nidx_rlm(1))
          nd = 1 + (kk - k_rlm) / nidx_rlm(1)
          ip_rtpm = 1 + (lp_rtm-1) * istep_rtm(2)                       &
     ...
     &                  + (irev_sr_rtm(ip_rtpm) - 1) * ncomp
!
          symp(kk+3*nkrv,lnum,np_smp) = WR(ipp_recv) * wp_rtm
          asmp(kk+3*nkrv,lnum,np_smp) = 0.0d0
        end do
!$omp end parallel do
      end if
!
      end subroutine set_vr_rtm_scl_sym_inner_omp
!
! -----------------------------------------------------------------------
! -----------------------------------------------------------------------
!
      subroutine cal_sp_rlm_vec_sym_inner_omp(nnod_rlm, nidx_rlm,       &
...
!$omp parallel 
      do jj = 1, n_jk_o
!$omp do private(kk,k_rlm,nd,ie_rlm,io_rlm,ie_send,io_send)
        do kk = 1, nkrv
          k_rlm = 1 + mod((kk-1),nidx_rlm(1))
          nd = 1 + (kk - k_rlm) / nidx_rlm(1)
!
...
!$omp do private(kk,k_rlm,nd,ie_rlm,ie_send)
        do kk = 1, nkrv
          k_rlm = 1 + mod((kk-1),nidx_rlm(1))
          nd = 1 + (kk - k_rlm) / nidx_rlm(1)
          ie_rlm = 1 + (2*jj+jst-2) * istep_rlm(2)                      &
...
!$omp end parallel
!
      end subroutine cal_sp_rlm_vec_sym_inner_omp
!
! -----------------------------------------------------------------------
!
      subroutine cal_sp_rlm_scl_sym_inner_omp                           &
...
!$omp end parallel
!
      end subroutine cal_sp_rlm_scl_sym_inner_omp
!
! -----------------------------------------------------------------------
! -----------------------------------------------------------------------
!
