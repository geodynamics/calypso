...
See \verb|t_ctl_data_4_platforms.f90| and \verb|ctl_data_platforms_IO.f90|.\\
...
\tt sym\_buoyancy &  $  -\left(\alpha_{T} T_{sym} +  \alpha_{C} C_{sym}  \right)\bvec{g}  $ \\
\tt asym\_buoyancy &  $ -\left(\alpha_{T} T_{asym} + \alpha_{C} C_{asym} \right)\bvec{g}  $ \\ \hline
%
...
\hline
\tt [Name] & field name & Description \\ \hline \hline
\tt temperature\_from\_CMB & Temperature from CMB average & $T - |T_{CMB}|$ \\
\tt composition\_from\_CMB & Composition variation from CMB average & $C - |C_{CMB}|$ \\
\tt pressure\_from\_CMB & Pressure from CMB average & $P - |P_{CMB}|$ \\
\hline
%
\end{tabular}
\end{center}
\label{table:field_components}
\end{table}
...
\paragraph{\tt coef\_4\_thermal\_buoyancy\_ctl}
\label{href_t:coef_4_thermal_buoyancy_ctl}
\verb|[Name] [Power]| \\
Coefficient for thermal buoyancy $- \alpha_{T} T \bvec{g}$ is defined by this array.
...
\item{\tt file: }      \\
Read reference temperature or heat source from data file defined by \hyperref[href_t:ref_field_file_name]{\tt [ref\_field\_file\_name]}
\item{\tt restart: }      \\
Read reference {\bf heat source} from restart data.
...
\subsubsection{\tt low\_temp\_ctl}
\hyperref[href_i:low_temp_ctl]{(Back to {\tt control\_MHD})}
...
\hyperref[href_i:high_temp_ctl]{(Back to {\tt control\_MHD})}

\paragraph{\tt depth}
\label{href_t:h_depth}
\verb|[RADIUS]| \\
Radius for reference temperature is defined by real.

\paragraph{\tt temperature}
\label{href_t:h_temperature}
\verb|[TEMPERATURE]| \\
Temperature for reference temperature is defined by real.

\subsubsection{\tt thermal\_diffusivity\_ctl}
\label{href_t:thermal_diffusivity_ctl}
Radial variation of thermal diffusivity is defined in this block. \\
\hyperref[href_i:thermal_diffusivity_ctl]{(Back to {\tt control\_MHD})}

\paragraph{\tt radial\_variation\_ctl}
\label{href_t:radial_variation_ctl}
\verb|[VARIATION_TYPE]| \\
...
\item{\tt reduction\_at\_ICB: }   \\
Set reduced diffusivity at ICB due to modeled latent heat. The diffusivity profile is defined by \hyperref[href_t:ICB_reduction_radius]{\tt ICB\_reduction\_radius}, \hyperref[href_t:ICB_reduction_ratio]{\tt ICB\_reduction\_ratio}, \\
and \hyperref[href_t:ICB_reduction_width]{\tt ICB\_reduction\_width}.
\item{\tt list\_in\_control: }   \\
Thermal diffusivity variation is defined by array \hyperref[href_t:diffusivity_list_ctl]{\tt diffusivity\_list\_ctl}.
\item{\tt file: }   \\
Thermal diffusivity variation is defined by data in a file name {\tt [FILE\_NAME]} defined by \hyperref[href_t:variation_file_name]{\tt variation\_file\_name}.
\end{description}
%

\paragraph{\tt variation\_file\_name}
\label{href_t:variation_file_name}
\verb|[FILE_NAME]| \\
File name for diffusivity variation data {\tt [FILE\_NAME]} is defined as text. The radial variation data can be choose the same file for the reference scalar or heat source data file. This parameter is used for the by {\tt file} mode in {\tt [VARIATION\_TYPE]}.

\paragraph{\tt diffusivity\_list\_ctl}
\label{href_t:diffusivity_list_ctl}
\verb|[RADIUS]  [RATIO]| \\
List of radial diffusivity variation as two reals array. Feature radius {\tt [RADIUS]} and diffusivity at the point {\tt [RATIO]} are defined. The diffusivity is linearly interpolated between the radial points. This parameter is used for the by {\tt list\_in\_control} mode in {\tt [VARIATION\_TYPE]}.


\paragraph{\tt ICB\_reduction\_radius}
\label{href_t:ICB_reduction_radius}
\verb|[RADIUS]| \\
Radius of thermal diffusivity at the point where the minimum diffusivity. This parameter is used for the by {\tt reduction\_at\_ICB} mode in {\tt [VARIATION\_TYPE]}.

\paragraph{\tt ICB\_reduction\_ratio}
\label{href_t:ICB_reduction_ratio}
\verb|[RATIO]| \\
Ratio of thermal diffusivity at the point where the minimum diffusivity defined by \\
{\tt ICB\_reduction\_radius} to the other area. This parameter is used for the by \\
{\tt reduction\_at\_ICB} mode in {\tt [VARIATION\_TYPE]}.


\paragraph{\tt ICB\_reduction\_width}
\label{href_t:ICB_reduction_width}
\verb|[WIDTH]| \\
Width of linearly decreasing the thermal diffusivity from the point where the minimum diffusivity. This parameter is used for the by {\tt reduction\_at\_ICB} mode in {\tt [VARIATION\_TYPE]}.
...
or {\tt thermal\_nusselt\_number\_prefix}}
\label{href_t:nusselt_number_prefix}
\verb|[nusselt_number_prefix]| \\
File prefix for thermal Nusselt number data at ICB and CMB \verb|[file_prefix]| is defined by Text. Program add {\tt .dat} or {\tt .dat.gz} extension after this file prefix. If this file prefix is not defined, Nusselt number data file is not generated. \\
{\bf CAUTION: Nusselt number is not evaluated if heat source exists.}

\paragraph{{\tt nusselt\_number\_format} \\ 
or {\tt thermal\_nusselt\_number\_format}}
\label{href_t:nusselt_number_format}
\verb|[file_format]| \\
File format for thermal Nusselt number data \verb|[file_prefix].dat|. If \verb|[file_format]| is \verb|gzip|, Nusselt number data is compressed by zlib.  If the Nusselt number data file exist, this setting is ignored and append data in the existing file.

\paragraph{\tt comp\_nusselt\_number\_prefix}
\label{href_t:comp_nusselt_number_prefix}
\verb|[nusselt_number_prefix]| \\
File prefix for compositional Nusselt number data at ICB and CMB \verb|[file_prefix]| is defined by Text. Program add {\tt .dat} or {\tt .dat.gz} extension after this file prefix. If this file prefix is not defined, Nusselt number data file is not generated. \\
{\bf CAUTION: Nusselt number is not evaluated if composition source exists.}

\paragraph{\tt comp\_nusselt\_number\_format}
\label{href_t:comp_nusselt_number_format}
\verb|[file_format]| \\
File format for compositional Nusselt number data \verb|[file_prefix].dat|. If \verb|[file_format]| is \verb|gzip|, Nusselt number data is compressed by zlib.  If the Nusselt number data file exist, this setting is ignored and append data in the existing file.

\paragraph{\tt CMB\_average\_file\_prefix}
\label{href_t:CMB_average_file_prefix}
\verb|[file_prefix]| \\
File prefix for CMB average data \verb|[file_prefix]| is defined by Text. Program add {\tt .dat} or {\tt .dat.gz} extension after this file prefix. If this file prefix is not defined, CMB average data file is not generated. \\

\paragraph{\tt CMB\_average\_file\_format}
\label{href_t:CMB_average_file_format}
\verb|[file_format]| \\
File format for CMB average data \verb|[file_prefix].dat|. If \verb|[file_format]| is \verb|gzip|,  CMB average data is compressed by zlib.  If the  CMB average data file exist, this setting is ignored and append data in the existing file.
