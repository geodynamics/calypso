!>@file   sph_exp_nod_insulate_CMB.f90
!!@brief  module sph_exp_nod_insulate_CMB
!!
!!@author H. Matsui
...
!!     &          r_CMB, n_point, d_rj_fld, d_rj_rot)
!!        integer(kind = kint), intent(in) :: jmax, kr_out
!!        real(kind = kreal), intent(in) :: g_sph_rj(jmax,13)
!!        real(kind = kreal), intent(in) :: r_CMB(0:2)
!!        integer(kind = kint), intent(in) :: n_point
...
!
      module sph_exp_nod_insulate_CMB
!
      use m_precision
      use m_constants
!
...
!$omp parallel do private(inod,i_n1,i_n2,j,d1s_dr1,d2s_dr2,d1t_dr1)
      do j = 1, jmax
        ...
        i_n2 = i_n1 - jmax
!
        d1s_dr1 = - g_sph_rj(j,1) * r_CMB(1) * d_rj_fld(inod,1)
...
!$omp parallel do private(inod,d1t_dr1)
      do j = 1, jmax
        inod = j + (kr_out-1) * jmax
        d1t_dr1 = - g_sph_rj(j,1) * r_CMB(1) * d_rj_fld(inod,3)
!
...
! -----------------------------------------------------------------------
!
      end module sph_exp_nod_insulate_CMB
