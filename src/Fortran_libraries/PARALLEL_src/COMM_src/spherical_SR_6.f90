!>@file   spherical_SR_6.f90
!!@brief  module spherical_SR_6
!!
!!@author H. Matsui
...
!$omp do private(k,j)
        do k= istart, iend
          j = inod_import(k)
          X_new(isix*j-5) = WR(isix*k-5)
          X_new(isix*j-4) = WR(isix*k-4)
          X_new(isix*j-3) = WR(isix*k-3)
          X_new(isix*j-2) = WR(isix*k-2)
          X_new(isix*j-1) = WR(isix*k-1)
          X_new(isix*j  ) = WR(isix*k  )
        end do
!$omp end do nowait
      end do
!$omp end parallel
...
!
      end subroutine sph_send_recv_6
!
! ----------------------------------------------------------------------
!
      end module spherical_SR_6
