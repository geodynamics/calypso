!>@file   spherical_SR_2.f90
!!@brief  module spherical_SR_2
!!
!!@author H. Matsui
...
!!
!!@verbatim
!!      subroutine sph_send_recv_2(nnod_org, nnod_new,                  &
...
!$omp do private(k,j)
        do k= istart, iend
          j = inod_import(k)
          X_new(itwo*j-1) = WR(itwo*k-1)
          X_new(itwo*j  ) = WR(itwo*k  )
        end do
!$omp end do nowait
      end do
!$omp end parallel
...
!
      end subroutine sph_send_recv_2
!
! ----------------------------------------------------------------------
!
      end module spherical_SR_2
