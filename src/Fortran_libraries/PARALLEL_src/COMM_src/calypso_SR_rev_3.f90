!>@file   calypso_SR_rev_3.f90
!!@brief  module calypso_SR_rev_3
!!
!!@author H. Matsui
!!@date Programmed in March, 2013
!
!>@brief  vector components data communication
!!@n      using reverse import table
!!
!!@verbatim
!!      subroutine calypso_send_recv_rev_3(nnod_org, nnod_new,          &
!!     &                           npe_send, isend_self,                &
!!     &                           id_pe_send, istack_send, inod_export,&
!!     &                           npe_recv, irecv_self,                &
!!     &                           id_pe_recv, istack_recv, irev_import,&
!!     &                           X_org, X_new)
!!      subroutine calypso_send_recv_rev_3x3(nnod_org, nnod_new,        &
!!     &                           npe_send, isend_self,                &
!!     &                           id_pe_send, istack_send, inod_export,&
!!     &                           npe_recv, irecv_self,                &
!!     &                           id_pe_recv, istack_recv, irev_import,&
...
!C
      do neib = 1, ncomm_send
        ist= 3 * istack_send(neib-1) + 1
        num  = 3 * (istack_send(neib  ) - istack_send(neib-1))
...
!C-- RECEIVE
      if(ncomm_recv .gt. 0) then
        do neib= 1, ncomm_recv
          ist= 3 * istack_recv(neib-1) + 1
          num  = 3 * (istack_recv(neib  ) - istack_recv(neib-1))
...
!
      if (isend_self .eq. 1) then
        ist_send= 3 * istack_send(npe_send-1)
        ist_recv= 3 * istack_recv(npe_recv-1)
        num = 3*(istack_send(npe_send) - istack_send(npe_send-1))
!$omp parallel do
        do i = 1, num
...
!
      end subroutine calypso_send_recv_rev_3
!
! ----------------------------------------------------------------------
!
      subroutine calypso_send_recv_rev_3x3(nnod_org, nnod_new,          &
...
!
!
      call resize_work_sph_SR((ithree*ithree), npe_send, npe_recv,      &
     &    istack_send(npe_send), istack_recv(npe_recv))
!
      ncomm_send = npe_send - isend_self
      ncomm_recv = npe_recv - irecv_self
...
!C
      do neib = 1, ncomm_send
        ist= 9 * istack_send(neib-1) + 1
        num  = 9 * (istack_send(neib  ) - istack_send(neib-1))
        call MPI_ISEND(WS(ist), num, CALYPSO_REAL,                      &
     &      id_pe_send(neib), 0, CALYPSO_COMM, req1(neib), ierr_MPI)
      end do
!C
!C-- RECEIVE
      if(ncomm_recv .gt. 0) then
        do neib= 1, ncomm_recv
          ist= 9 * istack_recv(neib-1) + 1
          num  = 9 * (istack_recv(neib  ) - istack_recv(neib-1))
          call MPI_IRECV(WR(ist), num, CALYPSO_REAL,                    &
     &      id_pe_recv(neib), 0, CALYPSO_COMM, req2(neib), ierr_MPI)
        end do
!
...
!
      if (isend_self .eq. 1) then
        ist_send= 9 * istack_send(npe_send-1)
        ist_recv= 9 * istack_recv(npe_recv-1)
        num = 9*(istack_send(npe_send) - istack_send(npe_send-1))
!$omp parallel do
        do i = 1, num
          WR(ist_recv+i) = WS(ist_send+i)
        end do
...
!
      call set_from_recv_buf_rev_3x3(nnod_new, istack_recv(npe_recv),   &
     &    irev_import, WR, X1_new, X2_new, X3_new)
!
      end subroutine calypso_send_recv_rev_3x3
!
! ----------------------------------------------------------------------
!
      end module calypso_SR_rev_3
