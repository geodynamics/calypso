!>@file   spherical_SR_rev_2.f90
!!@brief  module spherical_SR_rev_2
!!
!!@author H. Matsui
!!@date Programmed in March, 2013
!
!>@brief  Two components data communication
!!@n      for spherical harmonics transform using reverse import table
!!
!!@verbatim
!!      subroutine sph_send_recv_by_rev_2(nnod_org, nnod_new,           &
...
!$omp parallel do private(k,j)
      do k = 1, istack_send(npe_send)
        j = inod_export(k)
        WS(itwo*k-1)= X_org(itwo*j-1)
        WS(itwo*k  )= X_org(itwo*j  )
      end do
!$omp end parallel do
!C
...
!
      if (isend_self .eq. 1) then
        ist_send= itwo * istack_send(npe_send-1)
        ist_recv= itwo * istack_recv(npe_recv-1)
        inum = itwo*(istack_send(npe_send) - istack_send(npe_send-1))
!$omp parallel do
        do i = 1, inum
...
!$omp parallel do private(j,k)
      do k = 1, nnod_new
        j = irev_import(k)
        X_new(itwo*k-1) = WR(itwo*j-1)
        X_new(itwo*k  ) = WR(itwo*j  )
      end do
!$omp end parallel do
!
...
!
      end subroutine sph_send_recv_by_rev_2
!
! ----------------------------------------------------------------------
!
      end module spherical_SR_rev_2
