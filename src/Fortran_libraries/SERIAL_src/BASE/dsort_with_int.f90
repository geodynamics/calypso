...
!C***DESCRIPTION
!C
!C   DSORT_W_INT sorts array DX and optionally makes the same interchanges in
...
!C***END PROLOGUE  DSORT_W_INT
!
      module dsort_with_int
!
      use m_precision
!
      implicit none
...
!-----------------------------------------------------------------------
!
      SUBROUTINE DSORT_W_INT(DX, IY, N, KFLAG)
!
!C     .. Scalar Arguments ..
...
!C     .. Local Scalars ..
      ...
      INTEGER(kind = kint) :: TY, TTY
      INTEGER(kind = kint) :: I, IJ, J, K, KK, L, M, NN
!C     .. Local Arrays ..
...
!C     .. Intrinsic Functions ..
      INTRINSIC ABS, INT
!C***FIRST EXECUTABLE STATEMENT  DSORT_W_INT
      NN = N
      IF (NN .LT. 1) THEN
!         CALL XERMSG ('SLATEC', 'DSORT_W_INT',
...
!C
  ...
  200    CONTINUE
      ENDIF
      RETURN
      END SUBROUTINE DSORT_W_INT
!
!-----------------------------------------------------------------------
!
      end module dsort_with_int
