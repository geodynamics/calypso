...
!!     &          sph_bc_S, fdm2_center, r_coef, k_ratio, dk_dr,        &
!!     &          band_s00_poisson)
!!      subroutine sel_sph_r_mat_poisson_fixBC                          &
!!     &         (flag_val_diffuse, sph_rj, sph_bc_S, fdm2_center,      &
!!     &          k_ratio, dk_dr, band_s00_poisson)
...
!
!
      if     (sph_bc_S%iflag_icb .eq. iflag_sph_fill_center) then
        if(flag_val_diffuse) then
          call add_scl_val_dfse_mat_fill_ctr                            &
     &       (sph_rj%nidx_rj(1), sph_bc_S%r_ICB,                        &
     ...
     &        one, band_s00_poisson%mat)
      ...
      else if(sph_bc_S%iflag_icb .eq. iflag_sph_fix_center) then
        if(flag_val_diffuse) then
     ...
     &        fdm2_center%dmat_fix_fld, one, band_s00_poisson%mat)
      ...
      else if (sph_bc_S%iflag_icb .eq. iflag_fixed_flux                 &
     &    .or. sph_bc_S%iflag_icb .eq. iflag_evolve_flux) then
        call add_fix_flux_icb_poisson00_mat                             &
     &     (sph_rj%nidx_rj(1), sph_bc_S%kr_in,                          &
     &      sph_bc_S%fdm2_fix_dr_ICB, r_coef, band_s00_poisson%mat)
!      else if (sph_bc_S%iflag_icb .eq. iflag_fixed_field               &
!     &    .or. sph_bc_S%iflag_icb .eq. iflag_evolve_field) then
      else
        call set_fix_fld_icb_poisson00_mat                              &
     &     (sph_rj%nidx_rj(1), sph_bc_S%kr_in, band_s00_poisson%mat)
      end if
!
!
      flag_undefined = .TRUE.
      if(sph_bc_S%iflag_cmb .eq. iflag_fixed_flux                       &
     &    .or. sph_bc_S%iflag_cmb .eq. iflag_evolve_flux) then
        if(      sph_bc_S%iflag_icb .eq. iflag_sph_fix_center           &
     &      .or. sph_bc_S%iflag_icb .eq. iflag_fixed_field              &
     &      .or. sph_bc_S%iflag_icb .eq. iflag_evolve_field) then
          call add_fix_flux_cmb_poisson00_mat                           &
     &       (sph_rj%nidx_rj(1), sph_bc_S%kr_out,                       &
     &        sph_bc_S%fdm2_fix_dr_CMB, r_coef, band_s00_poisson%mat)
          flag_undefined = .FALSE.
        end if
      end if
!
!      else if (sph_bc_S%iflag_cmb .eq. iflag_fixed_field               &
!     &    .or. sph_bc_S%iflag_cmb .eq. iflag_evolve_field) then
      if(flag_undefined) then
        call set_fix_fld_cmb_poisson00_mat                              &
     &     (sph_rj%nidx_rj(1), sph_bc_S%kr_out, band_s00_poisson%mat)
      end if
!
...
!
!
      if     (sph_bc_S%iflag_icb .eq. iflag_sph_fill_center             &
     &   .or. sph_bc_S%iflag_icb .eq. iflag_sph_fix_center) then
        if(flag_val_diffuse) then
          call add_scl_val_diffuse_mat_fix_ctr                          &
     &       (sph_rj%nidx_rj(1), sph_bc_S%r_ICB,                        &
     &        fdm2_center%dmat_fix_fld, one, k_ratio(1), dk_dr(1),      &
