...
!
     ...
     &      rj_fld%n_point, rj_fld%d_fld(1,ipol%base%i_velo),           &
     &      rj_fld%d_fld(1,ipol%forces%i_h_advect))
      end if
!
     ...
     &      rj_fld%n_point, rj_fld%d_fld(1,ipol%base%i_velo),           &
     &      rj_fld%d_fld(1,ipol%forces%i_c_advect))
      end if
!
...
!   ----  Lead advection of reference field
!
      iflag = ipol%forces%i_h_advect * ipol%grad_fld%i_grad_temp        &
     &       * refs%iref_grad%i_grad_temp
      if(iflag .gt. 0) then
        call set_sphere_average_scalar(sph_rj, MHD_prop%ref_param_T,    &
     &      rj_fld%d_fld(1,ipol%grad_fld%i_grad_temp),                  &
     &      refs%ref_field%d_fld(1,refs%iref_grad%i_grad_temp),         &
...
!
!
      iflag = ipol%forces%i_c_advect * ipol%grad_fld%i_grad_composit    &
     &       * refs%iref_grad%i_grad_composit
      if(iflag .gt. 0) then
        call set_sphere_average_scalar(sph_rj, MHD_prop%ref_param_C,    &
     &      rj_fld%d_fld(1,ipol%grad_fld%i_grad_composit),              &
     &      refs%ref_field%d_fld(1,refs%iref_grad%i_grad_composit),     &
     &      ref_grad_local, ref_grad_S)
        call add_reference_scalar_advect(sph_rj, leg,                   &
