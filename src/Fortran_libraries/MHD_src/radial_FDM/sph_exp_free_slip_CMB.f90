...
!!
!!@verbatim
!!      subroutine cal_sph_nod_cmb_free_v_and_w                         &
!!     &         (jmax, kr_out, fdm2_free_vp_CMB, fdm2_free_vt_CMB,     &
...
!!     &          fdm2_free_vp_CMB, n_point, d_rj_fld)
!!        integer(kind = kint), intent(in) :: jmax, kr_out
!!        real(kind = kreal), intent(in)  :: fdm2_free_vp_CMB(-1:1,3)
!!        integer(kind = kint), intent(in) :: n_point
!!        real (kind=kreal), intent(inout) :: d_rj_fld(n_point,3)
!!      subroutine cal_sph_nod_cmb_free_rot2(jmax, g_sph_rj, kr_out,    &
...
!!        real (kind=kreal), intent(inout) :: d_rj_rot(n_point,3)
!!      subroutine cal_sph_nod_cmb_free_diffuse2(jmax, g_sph_rj, kr_out,&
!!     &          r_CMB, fdm2_free_vp_CMB, fdm2_free_vt_CMB, coef_d,    &
...
!!      subroutine cal_sph_nod_cmb_free_w_diffuse2(jmax, g_sph_rj,      &
!!     &          kr_out, r_CMB, fdm2_fix_fld_CMB, fdm2_free_vt_CMB,    &
!!     &          coef_d, n_point, d_rj_vort, d_rj_w_diffuse)
!!        integer(kind = kint), intent(in) :: jmax, kr_out
!!        real(kind = kreal), intent(in) :: g_sph_rj(jmax,13)
!!        real(kind = kreal), intent(in) :: r_CMB(0:2)
!!        real(kind = kreal), intent(in) :: coef_d
...
!!        integer(kind = kint), intent(in) :: n_point
!!        real(kind = kreal), intent(in) :: d_rj_vort(n_point,3)
...
!$omp parallel do private(inod,i_n1,i_n2,j,d1s_dr1,d2s_dr2,d1t_dr1)
      do j = 1, jmax
        ...
        i_n2 = i_n1 - jmax
!
        d1s_dr1 =  fdm2_free_vp_CMB(-1,2) * d_rj_fld(i_n1,1)
        d2s_dr2 =  fdm2_free_vp_CMB(-1,3) * d_rj_fld(i_n1,1)
        d1t_dr1 =  fdm2_free_vt_CMB(-1,2) * d_rj_fld(i_n1,3)            &
     &           + fdm2_free_vt_CMB( 0,2) * d_rj_fld(inod,3)
!
...
!
        d1s_dr1 =  fdm2_free_vp_CMB(-1,2) * d_rj_fld(i_n1,1)            &
     &           + fdm2_free_vp_CMB( 0,2) * d_rj_fld(inod,1)
!
        d_rj_fld(inod,1) = zero
        d_rj_fld(inod,2) = d1s_dr1
      end do
...
!
        d2s_dr2 =  fdm2_free_vp_CMB(-1,3) * d_rj_fld(i_n1,1)            &
     &           + fdm2_free_vp_CMB( 0,3) * d_rj_fld(inod,1)
        d1t_dr1 =  fdm2_free_vt_CMB(-1,2) * d_rj_fld(i_n1,3)            &
     &           + fdm2_free_vt_CMB( 0,2) * d_rj_fld(inod,3)
!
        d_rj_rot(inod,1) =  d_rj_fld(inod,2)
        d_rj_rot(inod,2) =  d1t_dr1
        d_rj_rot(inod,3) = -d2s_dr2                                     &
...
!
        d2s_dr2 =  fdm2_free_vp_CMB(-1,3) * d_rj_fld(i_n1,1)            &
     &           + fdm2_free_vp_CMB( 0,3) * d_rj_fld(inod,1)
        d2t_dr2 =  fdm2_free_vt_CMB(-1,3) * d_rj_fld(i_n1,3)            &
     &           + fdm2_free_vt_CMB( 0,3) * d_rj_fld(inod,3)
!
...
!$omp parallel do private(inod,i_n1,i_n2,j,d2s_dr2,d2t_dr2)
      do j = 1, jmax
        ...
        i_n2 = i_n1 - jmax
!
        d2s_dr2 =  fdm2_free_vt_CMB(-1,3) * d_rj_vort(i_n1,1)           &
     &           + fdm2_free_vt_CMB( 0,3) * d_rj_vort(inod,1)
        d2t_dr2 =  fdm2_fix_fld_CMB(-2,3) * d_rj_vort(i_n2,3)           &
     &           + fdm2_fix_fld_CMB(-1,3) * d_rj_vort(i_n1,3)           &
     &           + fdm2_fix_fld_CMB( 0,3) * d_rj_vort(inod,3)
!
        d_rj_w_diffuse(inod,1) =  coef_d * (d2s_dr2                     &
     &               - g_sph_rj(j,3)*r_CMB(2) * d_rj_vort(inod,1))
        d_rj_w_diffuse(inod,3) =  coef_d * (d2t_dr2                     &
     &               - g_sph_rj(j,3)*r_CMB(2) * d_rj_vort(inod,3))
      end do
!$omp end parallel do
!
