...
!
      do m = -1, 1
        if(idx_rj_degree_one(m) .gt. 0) then
          iICB = idx_rj_degree_one(m) + (kr_in-1) * nidx_rj(2)
          d_rj_fld(iICB,3) = bc_l1_tmp(m)
          do k = 1, kr_in-1
            inod = idx_rj_degree_one(m) + (k-1) * nidx_rj(2)
            d_rj_fld(inod,3) = bc_l1_tmp(m)*r_ICB(2)                    &
...
!$omp parallel do private(inod,i_p1,i_p2,j,d2s_dr2,d1t_dr1)
      do j = 1, jmax
        ...
        i_p2 = i_p1 + jmax
!
        d2s_dr2 =  fdm2_fix_dr_ICB( 0,3) * d_rj_fld(inod,1)             &
...
!$omp parallel do private(inod,i_p1,i_p2,j,d2s_dr2,d2t_dr2)
      do j = 1, jmax
        ...
        i_p2 = i_p1 + jmax
!
        d2s_dr2 =  fdm2_fix_dr_ICB( 1,3) *  d_rj_fld(i_p1,1)
        d2t_dr2 =  fdm2_fix_fld_ICB( 1,3) * d_rj_fld(i_p1,3)            &
