...
! -----------------------------------------------------------------------
!
      subroutine write_merged_ucd_fields(id_ucd, ucd, m_ucd)
!
      use t_ucd_data
!
...
!
      if(my_rank .eq. 0) then
        call write_udt_field_header(id_ucd,                             &
     &      ucd%num_field, ucd%num_comp, ucd%phys_name)
      end if
!
      call write_merged_udt_field(id_ucd,                               &
...
! -----------------------------------------------------------------------
!
      subroutine write_merged_ucd_mesh(id_ucd, ucd, m_ucd)
!
...
!
      if(my_rank .eq. 0) then
        call write_udt_mesh_header(id_ucd,                              &
     &      m_ucd%istack_merged_intnod(nprocs),                         &
     &      m_ucd%istack_merged_ele(nprocs), ucd%ntot_comp)
      end if
!
      call write_merged_udt_field(id_ucd, ucd%nnod, n_vector, ucd%xx,   &
     &    m_ucd%istack_merged_nod, m_ucd%istack_merged_intnod)
      call write_merged_ucd_connect(id_ucd,                             &
     &    ucd%nnod_4_ele, ucd%nnod_4_ele, ucd%ie,                       &
...
! -----------------------------------------------------------------------
!
      subroutine write_merged_ucd_connect(id_ucd, nele, nnod_ele,       &
     &          ie, istack_numele)
!
...
!
      integer(kind = kint) :: ip, num4, isend_rank, k1
      integer(kind = kint_gl) :: iele, num, i
!
!
      if(my_rank .eq. 0) then
        ...
        do k1 = 1, nnod_ele
          do iele = 1, istack_numele(ione)
            i = iele + (k1-1) * istack_numele(ione)
            ie_single_ucd(i) = ie(iele,k1)
          end do
        end do
!
        call write_ucd_mesh_connect(id_ucd, istack_numele(1), nnod_ele, &
     &      istack_numele(1), iele_single_ucd(1), ie_single_ucd(1))
      end if
!
      do ip = 2, nprocs
        isend_rank = ip- 1
...
!
      use m_merged_ucd_data
!
      integer(kind = kint), intent(in) ::  id_ucd, ncomp_field
...
!
      ...
      integer(kind = kint_gl) :: inod, nnod, num
!
!
      if(my_rank .eq. 0) then
...
!
        nnod = istack_numnod(1) - istack_numnod(0)
        call write_ucd_field_data(id_ucd, nnod, ncomp_field,            &
...
!
          ...
          num =  int(istack_intnod(ip) - istack_intnod(ip-1))
          do inod = 1, num
